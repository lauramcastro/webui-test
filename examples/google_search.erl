...
-export([run/0, run/1, setup/0, teardown/0]).
-include("webui.hrl").
...
-spec run() -> boolean().
run() ->
    webui_model:run(?MODULE, ?URL, 1).

%% @doc
%%      Runs model for Google search a number of times.
%% @end
-spec run(N :: pos_integer()) -> boolean().
run(N) when N > 0 ->
    webui_model:run(?MODULE, ?URL, N).
